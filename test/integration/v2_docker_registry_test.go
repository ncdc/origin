...
`
	...
	if _, err := openshift.Client.ImageRepositories(testNamespace).Create(&repo); err != nil {
	...
	putUrl := fmt.Sprintf("http://127.0.0.1:5000/v2/%s/%s/manifests/%s", testNamespace, repo.Name, "latest")
	...
	url := fmt.Sprintf("http://127.0.0.1:5000/v2/%s/%s/manifests/%s", testNamespace, repo.Name, dgst.String())
	...
	image, err := openshift.Client.ImageStreamImages(testNamespace).Get(repo.Name, dgst.String())
	if err != nil {
	...
	if e, a := "127.0.0.1:5000/integration-test/test@"+dgst.String(), image.DockerImageReference; e != a {
...
func getTags(repoName string) ([]string, error) {
	url := fmt.Sprintf("http://127.0.0.1:5000/v2/%s/%s/tags/list", testNamespace, repoName)
	resp, err := http.Get(url)
	if err != nil {
